...
class MonsterFighterA:
    ...
    def attack_small_or_elite(self, x1, y1, x2, y2):
        ...
        should_run_to_qianjin = self.attacker.attack_small_or_elite(x1, y1, x2, y2)
    ...
    def attack_boss(self, x1, y1, x2, y2):
        print("attack_boss")
        should_run_to_qianjin = self.attacker.attack_boss(x1, y1, x2, y2)
        if should_run_to_qianjin:
            print(" attack_boss should_run_to_qianjin")
            self.run_to_forward()

    def pickup_boss_drops(self):
    ...
    def detect_smsd(self):
        ...
        while not self.gui.stop_event.is_set():
            ...
            image_array = self.yolo_util.capture_screen()
            if self.yolo_util.check_exist(DetectionTarget.SMSD, image_array):
                cx, cy = self.yolo_util.get_positions(DetectionTarget.CAN_HEN, image_array)
                # 购买残痕
                self.utils.release_keyboard()
    ...
    def fight_monsters(self, frame, gray_frame):
        ...
        for monster_name, x1, y1, x2, y2 in detected_monsters:
            ...
            if monster_name == 'forward':
                ...
                #检测到boss
                self.attack_boss(x1, y1, x2, y2)
                self.last_execute_time = time.time()
            elif monster_name == 'monster':
                #检测到小怪
                self.attack_small_or_elite(x1, y1, x2, y2)
                self.last_execute_time = time.time()
